...
while(True):
    ...
    try:
        ...
        while(cap.isOpened()):
            ...
            cv2.line(image_resized,tuple(bottom_left),tuple(bottom_right), (255, 0, 0), 5)
            cv2.line(image_resized,tuple(bottom_right),tuple(top_right), (255, 0, 0), 5)
            cv2.line(image_resized,tuple(top_left),tuple(bottom_left), (255, 0, 0), 5)
            cv2.line(image_resized,tuple(top_left),tuple(top_right), (255, 0, 0), 5)
            copied = np.copy(image_resized)
            ...
            scores, boxes, num_detections, classes= output_tensors            
            print(scores.ndim, boxes.ndim, num_detections.ndim, classes.ndim)
            print(boxes)
            print(scores)
            ...
            Result = np.array(image_resized[ymin:ymax,xmin:xmax])

            ymin_str='y min  = %.2f '%(ymin)
            ymax_str='y max  = %.2f '%(ymax)
            xmin_str='x min  = %.2f '%(xmin)
            xmax_str='x max  = %.2f '%(xmax)
            cv2.putText(image_resized,ymin_str, (50, 50),cv2.FONT_HERSHEY_SIMPLEX,0.6,(255,0,0),2)
            cv2.putText(image_resized,ymax_str, (50, 70),cv2.FONT_HERSHEY_SIMPLEX,0.6,(255,0,0),2)
            cv2.putText(image_resized,xmin_str, (50, 90),cv2.FONT_HERSHEY_SIMPLEX,0.6,(255,0,0),2)
            cv2.putText(image_resized,xmax_str, (50, 110),cv2.FONT_HERSHEY_SIMPLEX,0.6,(255,0,0),2)
